...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  return Math.abs(Math.sqrt(Math.abs((x2 - x1) ** 2 + (y2 - y1) ** 2)));
}
...
function toNumber(value, def) {
  /* if (Number.isNumber(value) === true) return value; */
  if (Number.isNaN(Number(value)) === true) return def;
  return Number(value);
}
...
function isPowerOfTwo(num) {
  if (Number.isInteger(Math.log2(num)) === true) return true;
  return false;
}
